...
while True:
    ...
    if hands and buttonPress is False:
        ...
        #value within annotation works (limit from the half screen to end)
        xVal=int(np.interp(lmList[8][0],[width//2,width],[0,width]))
        yVal=int(np.interp(lmList[8][1],[150,height-150],[0,height]))
        indexFinger=xVal,yVal    

        #left and right moving slideq
        ...
        if fingers==[0,0,0,0,1]: #right
            ...
            if imgNum<len(pathImages)-1:
                ...
                imgNum+=1   
         # close the ppt    
        if fingers==[0,0,1,1,1]:#last three finger
            cv.destroyAllWindows()  
            break      

        #circle point on slide using index and middle finger
        if fingers==[0,1,1,0,0]:
            cv.circle(imgCurr,indexFinger,10,(0,0,255),cv.FILLED)
    ...
    for i in range(len(annotations)):
        for j in range(len(annotations[i])):
            if j!=0:
                cv.line(imgCurr,annotations[i][j-1],annotations[i][j],(0,0,255),6)       
